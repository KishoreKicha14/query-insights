...
public class ReportQueryBytesRequestHandler implements TransportRequestHandler<BytesTransportRequest> {
    ...
    private SearchQueryRecord deserialize(final BytesTransportRequest request) throws java.io.IOException {
        try (StreamInput in = request.bytes().streamInput()) {
            ...
            final String[] indices = new String[indexCount];
            ...
            // Index name(s) the query reads from, resolved by the SQL plugin from the query text.
            // Stored as a String[] to match how the native search listener sets INDICES
            // (request.indices()), so the dashboard's Indices column/row renders identically.
            if (indices.length > 0) {
            ...
            // Attach the user captured on the coordinator. The transient does not survive the
            // transport hop to this handler thread, so the user arrives on the wire and is rebuilt
            // here from the same string format the security plugin writes. setUserInfo() later
            // materializes USERNAME/USER_ROLES/BACKEND_ROLES from this, enabling RBAC filtering.
            final UserPrincipalContext userPrincipalContext = userInfo == null || userInfo.isEmpty()
                ? null
                : UserPrincipalContext.fromUserString(userInfo);

            return new SearchQueryRecord(timestampMillis, measurements, attributes, null, userPrincipalContext, null);
        }
    }
}

...
public class QueryInsightsService extends AbstractLifecycleComponent {
    ...
    public void putLiveQueryUserInfo(String taskId, UserPrincipalContext.UserPrincipalInfo userInfo) {
        ...
        if (liveQueryUserMap.size() >= USER_INFO_MAX_SIZE) {
            logger.warn("Live query user info map at capacity ({}). Skipping capture for task [{}]. "
                + "Consider increasing concurrency capacity if this persists.", USER_INFO_MAX_SIZE, taskId);
            return;
        }
